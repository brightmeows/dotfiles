...
description: 当编辑或审阅文本、发现 AI 生成痕迹（过度修辞、空洞强调词、公式化段落、过于确定的语气）时使用，使文字更自然、更像人写的。触发词：AI 味、润色、去模板化。即使未明确要求润色，只要检测到 AI 味就先加载。
allowed-tools:
  - Read
  - Write
  - Edit
...
5. **删除金句** - 如果听起来像可引用的语句，重写它（详见附录A模式16）
...
如需进一步降低 AI 检测率，可参考附录B中的三招构造策略（打破总-分-总、注入主观不确定性、增加信息密度）。
...
**问题：** AI 倾向在段落末尾添加一句可引用的总结性陈述，听起来像名人名言或广告语——“这不仅是……更是……”、“这标志着……”、“未来已来”等。这类句子本身没错，但密集出现是 AI 写作的显著特征。

**改写前：**
> 新的教育政策旨在减轻学生负担，提高教育质量。这不仅仅是政策的调整，更是教育理念的根本转变。

**改写后：**
> 新政策取消了小学一二年级的书面考试，要求三年级以上每学期最多一次统考。校长们对此反应不一：有的说“终于能腾出时间搞活动了”，有的担心中考成绩会下滑。

**关键：** 不是删除所有结论句，而是把空洞的修辞性结论替换为具体的事实或引述。
