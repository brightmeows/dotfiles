...
[层级与作用域](#层级与作用域)节判断层级归属，本节按对代理理解项目的重要程度降序排列，内容非必需，按需取用。
...
- **可放**：非标准目录布局说明
- **不放**：长篇架构概述、组件关系、数据流图
- **推荐行为**：完整架构链接 docs/ARCHITECTURE.md

#### 数据定义

- **可放**：关键数据模型说明
- **不放**：完整的数据库 schema、数据结构定义
- **推荐行为**：指向 ORM 定义或 docs/DATABASE.md

#### 第三方依赖 / 外部服务

- **可放**：已知兼容性问题、特殊配置说明、服务 URL
- **不放**：第三方 SDK 完整用法文档、API 密钥值

### 不推荐放 AGENTS.md

以下内容归属外部文档或工具配置，不应直接放入 AGENTS.md。
