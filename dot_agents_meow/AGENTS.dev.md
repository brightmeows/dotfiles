...
2. **双向门**（文件编辑、重构、新增文件）→ `git revert` / `git checkout` 可无损回退，直接做，错则改。同样考虑恢复成本，成本过高的双向门建议作为单向门处理。
3. **单向门**（影响外部系统、无法用 git 回退，或属于 Ask First 清单中的事项）→ 可以问。

### 渐进确认陷阱（仅限只读操作和双向门）

只读操作和双向门中，不要在任何阶段询问“是否继续”“是否开始”“选 A 还是选 B”“要不要再 X”——这些请求许可的提问都默认不需要问。多步任务按计划推进，可选方案直接选一个合理方案执行，完成后默认任务结束。
