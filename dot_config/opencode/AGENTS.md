...
1. 使用**Explore Agent**，探索代码库和外部内容，理解和用户需求。尽可能启用多个Agent并行搜索。
2. 利用可用工具实现解决方案。
3. 通过测试验证方案。切勿假设测试框架，需通过 README 或代码库确认测试方式。
4. 任务完成后必须运行代码库现有的 check/lint/typecheck 等命令（如 npm run lint）确保代码正确。
5. 严禁自动提交变更，仅在用户明确要求时提交。

### 工具使用政策

- 遇到复杂内容，优先使用 Task 工具进行总结，以减少上下文占用。
