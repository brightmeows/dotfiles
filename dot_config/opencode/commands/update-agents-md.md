...
先读取 `structuring-agents-md` skill，并读取该 skill 的所有参考内容。
...
一次同时启动多个检查子 agent。
- 提示词中应提示检查子 agent 加载 `structuring-agents-md` skill，且寻找尽可能的多的可优化点。
...
如果第一步发现问题，派修复子 agent 修复（加载 `structuring-agents-md` skill）。
禁止修改代码实际实现，可以改注释与文档。
